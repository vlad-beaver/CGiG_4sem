...
void DrawLightSphere(CDC& dc, double Radius, CMatrix& PView, CMatrix& PSourceLight, CRect RW, COLORREF Color, int Index)
{
	...
	double kLight;
	VSphere(0) = Radius;	                      
	CMatrix VR = SphereToCart(PView);       	  // вектор камеры
	CMatrix VS = SphereToCart(PSourceLight);	  // вектор света
	...
	for (double fi = 0; fi < 360.0; fi += df)		  //углы
		for (double q = 0; q <= 180.0; q += dq)		  
		{
			...
			VSphereNorm = VCart;				      
			double cos_RN = CosV1V2(VR, VSphereNorm);	//угол между камерой и нормалью 
													  
			if (cos_RN > 0)		// если видим сторону
			{
				PV(0) = VCart(0);
				PV(1) = VCart(1);
				PV(2) = VCart(2);
				...
				CMatrix VP = VS - VR;                 //вектор определяющий освещаемость поверхности   для глаз 
													     
				double cos_PN = CosV1V2(VP, VSphereNorm);	
														  
				if (cos_PN > 0)		// свет падает
				{
					if (Index == 0) 
						kLight = cos_PN;		//
					else if (Index == 1)
						kLight = CosV1V2(VSphereNorm, VS);		//угол между нормалью и светом - зеркальное
					Col = RGB(kLight*red, kLight*green, kLight*blue);
					dc.SetPixel((int)VCart(0), (int)VCart(1), Col);
				}
				else	// сторона не освещается
				{
					Col = RGB(0, 0, 0);
					dc.SetPixel((int)VCart(0), (int)VCart(1), Col);
				}
